...
for problem_dir in glob('../problems/p7'):
    problem = op.basename(problem_dir)
    for infile in glob(op.join(problem_dir, 'in6', 'input*.txt')):
        input_name = op.splitext(op.basename(infile))[0]
        outfile = op.join(problem_dir, 'out6', 'output' + input_name.split('input')[-1] + '.txt')
